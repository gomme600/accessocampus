...
class Lepton(object):
    ...
    def __init__(self, spi_dev="/dev/spidev0.0"):
        ...
        self.__capture_buf = np.zeros((Lepton.ROWS, Lepton.VOSPI_FRAME_SIZE), dtype=np.uint16)
    ...
    def capture(self, data_buffer=None, log_time=False, debug_print=False, retry_reset=True):
        ...
        if data_buffer is None:
            data_buffer = np.ndarray((Lepton.ROWS, Lepton.COLS), dtype=np.uint16)
        ...
        if debug_print:
            print("---")
            for i in range(Lepton.ROWS):
                fid = self.__capture_buf[i, 0]
                crc = self.__capture_buf[i, 1]
                fnum = fid & 0xFFF
